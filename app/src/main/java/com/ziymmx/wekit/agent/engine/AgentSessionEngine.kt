...
import com.ziymmx.wekit.agent.model.LlmClient
...
class AgentSessionEngine(
    ...
    fun runTurn(
        ...
        try {
            ...
            while (true) {
                ...
                val request = com.ziymmx.wekit.agent.model.LlmRequest(
    ...
    private suspend fun executeToolCall(
        ...
        if (tool.mode == com.ziymmx.wekit.agent.tool.ToolMode.MANUAL_APPROVAL) onAwaitingApproval(call.name)
        ...
        return when (decision) {
            is ApprovalDecision.Allowed -> {
                val status = when (tool.mode) {
                    com.ziymmx.wekit.agent.tool.ToolMode.MANUAL_APPROVAL -> ApprovalStatus.USER_APPROVED
                    com.ziymmx.wekit.agent.tool.ToolMode.SMART_APPROVAL -> ApprovalStatus.AI_APPROVED
    ...
    private fun parseArgs(argumentsJson: String): JsonObject =
        runCatching { com.ziymmx.wekit.agent.model.LlmJson.json.parseToJsonElement(argumentsJson).jsonObject }
